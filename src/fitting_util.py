...
functions = {
    ...
    'line': {'func': lambda p, x: np.array([(p[1] * dx) + p[0] for dx in x]),
             'pinit': [0., 1.]},
    'quadratic': {'func': lambda p, x: np.array(
        [(p[2] * dx * dx) + (p[1] * dx) + p[0] for dx in x]),
                  'pinit': [0., 1., 1.]},
    'step': {'func': lambda p, x: np.array([p[1] if dx <= p[0] else p[2] for dx in x]),
             'pinit': [5., 0.1, 1.]},
    'gaussian': {'func': lambda p, x: np.array(
        p[0] * np.exp(-np.power(x - p[1], 2) / (2 * np.power(p[2], 2)))),
                 'pinit': [1., 5., 1.]},
}


def fit_leastsq(p0: list, datax: list, datay: np.array, fitfunc: object,
                y_err: list = []) -> list:
